...
class StaticIncomeController extends \TCG\Voyager\Http\Controllers\VoyagerBreadController
{
    ...
    {
        $validKey = array('group_id','title','description','amount','debit_day_per_month','attachment','debited_at', 'user_id');
        ...
        foreach($validKey as $key){
            ...
            if($request->file('attachment') && $key == "attachment") {
                ...
                $path = Storage::disk('public')->put('static-income', $image);
                //dd($path);
                $new->put($key, $path);
            }
            if($key == 'user_id'){
    ...
    {
        $validKey = array('id','group_id','title','description','amount','debit_day_per_month','attachment','debited_at');
        ...
        if($request->file('attachment') && $updateStaticIncome->attachment 
            && Storage::disk('public')->exists($updateStaticIncome->attachment)) {
            Storage::disk('public')->delete($updateStaticIncome->attachment);
        }
    ...
    public function ajax_delete(Request $request){
        ...
        if($delete_id){
            ...
            if($staticIncome->attachment && Storage::disk('public')->exists($staticIncome->attachment)) { 
                Storage::disk('public')->delete($staticIncome->attachment); 
            }
    ...
    public function ajax_bulk_delete(Request $request){
        ...
        if(count($delete_ids) > 0){
            ...
            $staticIncomes = StaticIncome::whereIn('id', $delete_ids)->get();

            foreach($staticIncomes as $staticIncome){
                if($staticIncome->attachment && Storage::disk('public')->exists($staticIncome->attachment)) { 
                    Storage::disk('public')->delete($staticIncome->attachment); 
                }
            }
